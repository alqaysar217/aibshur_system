...
export function AdminSidebar({ isMobileOpen, setIsMobileOpen, isCollapsed }) {
    ...
    return (
      <Link href={href} onClick={() => isMobileOpen && setIsMobileOpen(false)} title={text}>
        ...
        >
          ...
          <span className={cn('transition-opacity duration-200', isCollapsed ? 'opacity-0 w-0' : 'opacity-100')}>{text}</span>
        </span>
      </Link>
    );
  };
  ...
  return (
    <aside className={cn(
      "fixed top-0 right-0 h-screen z-[101] bg-white border-l border-border flex flex-col transition-all duration-300 ease-in-out",
      // Mobile state
      "lg:relative",
      ...
      isCollapsed ? 'w-20' : 'w-64'
        ...
        <div className={cn("flex items-center h-20 border-b shrink-0 px-4 gap-3", isCollapsed && "justify-center")}>
             <Image src="https://i.postimg.cc/L8g1v4w1/absher-logo-2.png" alt="أبشر Logo" width={isCollapsed ? 40 : 100} height={isCollapsed ? 40 : 34} className="transition-all duration-300"/>
             <span className={cn("font-black text-lg text-foreground transition-opacity duration-200", isCollapsed ? 'opacity-0 w-0' : 'opacity-100')}>إدارة أبشر</span>
        </div>
        ...
        <div className="px-3 py-4 border-t shrink-0">
             <Link href="#">
                <span
                className={cn(
                    'flex items-center gap-4 px-4 py-3 font-bold rounded-lg transition-colors duration-200 text-red-500 bg-red-50 hover:bg-red-100',
                    isCollapsed && 'justify-center'
                )}
                >
                <LogOut className={'w-5 h-5 shrink-0'} />
                <span className={cn("transition-opacity duration-200", isCollapsed && "opacity-0 w-0")}>تسجيل الخروج</span>
                </span>
            </Link>
        </div>
    </aside>
